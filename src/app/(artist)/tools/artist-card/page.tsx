...
export default function ArtistCardPage() {
    ...
    return (
      <div className="max-w-4xl mx-auto pb-20 animate-entry">
        ...
        <div className="glass p-12 rounded-3xl text-center border border-border">
          ...
          <h1 className="text-3xl font-bold mb-4 text-textMain">{dict.common.availableInPremium}</h1>
          <p className="text-textMuted mb-8 max-w-lg mx-auto">
            {dict.common.artistCardManagement}
          </p>
  ...
  if (!topic || !description || !artistCardLink) {
    alert(dict.common.fillRequired);
    return;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 animate-entry">
      ...
      <h1 className="text-3xl font-bold mb-2 text-textMain">{dict.common.artistCardInteraction}</h1>
      <p className="text-textMuted mb-8">{dict.common.artistCardDesc}</p>

      <form onSubmit={handleSubmit} className="glass p-8 rounded-3xl space-y-6 mb-12">
        <div className="space-y-2">
          <label className="text-sm font-medium text-textMuted">{dict.common.requestTopic}</label>
          <Select
            options={TOPICS}
            value={topic}
            onChange={setTopic}
            placeholder={dict.common.selectTopic}
          />
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium text-textMuted">{dict.common.platform} *</label>
          <Select
            options={PLATFORMS}
            value={platform}
            onChange={setPlatform}
            placeholder={dict.common.selectPlatform}
          />
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium text-textMuted">{dict.common.description}</label>
          <textarea
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-textMuted">{dict.common.artistCardLink}</label>
          <Input
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium text-textMuted">{dict.common.attachments}</label>
          <Input
            placeholder={dict.common.attachmentsPlaceholder}
            value={attachment}
        ...
        <div className="flex items-center justify-end gap-4 pt-4">
          {success && (
            <div className="flex items-center text-success text-sm font-medium animate-in fade-in">
              ...
              {dict.common.requestSent}
            </div>
          )}
          <Button type="submit" disabled={loading} size="lg" className="rounded-xl">
            {loading ? dict.common.sending : dict.common.sendRequestBtn}
          </Button>
        </div>
      </form>

      <h2 className="text-2xl font-bold mb-6 text-textMain">{dict.common.myRequests}</h2>
      <div className="space-y-4">
        {requests.length === 0 ? (
          <div className="p-8 text-center text-textMuted glass rounded-2xl">{dict.common.noRequests}</div>
        ) : (
          requests.map((req) => (
            <div key={req.id} className="glass p-6 rounded-2xl border border-border">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-sm text-textMuted">{req.platform || dict.common.allPlatforms}</div>
                </div>
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                  ...
                  {req.status === 'DONE' ? dict.common.done : req.status === 'REJECTED' ? dict.common.rejected : dict.common.pending}
                </span>
              </div>
              ...
              <div className="text-xs text-textMuted flex gap-4">
                ...
                {req.artistCardLink && <a href={req.artistCardLink} target="_blank" rel="noreferrer" className="text-primary hover:underline">{dict.common.artistCard}</a>}
                {req.attachmentUrl && <a href={req.attachmentUrl} target="_blank" rel="noreferrer" className="text-primary hover:underline">{dict.common.attachment}</a>}
              </div>
            </div>
          ))
        )}
